...
class AttentionModelNew(nn.Module):
    ...
    def forward(self, records_u, is_train):
        ...
        for idx, record in enumerate(records_al):
            ...
            scores_hl = self.decoder_hl(hidden_long_al[idx].view(1, -1), Variable(torch.LongTensor(vid_candidates)).view(1, -1))
            scores_hs = self.decoder_hs(hidden_short_al[idx].view(1, -1), Variable(torch.LongTensor(vid_candidates)).view(1, -1))
            scores_d_all = self.get_scores_d_all(records_u, idx, vid_candidates, feature_al, is_train)
            if self.mod == 0:
                scores_merge = torch.cat((scores_u, scores_t, scores_hl, scores_hs, scores_d_all), 0).t()
                if is_train:
    ...
    def get_scores_d_all(self, records_u, idx_cur, vid_candidates, feature_al, is_train):  #id: current record id, want to predict record[id].vid_next
        ...
        for idx_r, record in enumerate(records_al):
            if idx_r == idx_cur + 1:
                ...
                continue
            coor_r = self.vid_coor_nor[records_u.records[idx_r].vid] #current coor of idx_r
            dist = np.sqrt(np.sum((coor_cur - coor_r) ** 2)) #distance
    ...
    def get_vids_candidate(self, rid, vids_visited, vid_true=None, is_train=True):
        if rid in self.rid_sampling_info:
            vids, probs_pop, probs_dist = self.rid_sampling_info[rid]
        else:
            nbs = set()
            vid_dist = {}
            for vid_visited in vids_visited:
                dists, vids = self.tree.query([self.vid_coor_nor[vid_visited]], 50)
                for i, vid in enumerate(vids[0]):
                    vid = vids[0, i]
                    if (not is_train) or (is_train and vid != vid_true):
                        nbs.add(vid)
                        if vid not in vid_dist or dists[0, 1] < vid_dist[vid]:
                            vid_dist[vid] = dists[0, i]
            vids = list(nbs)
            probs_pop = np.array([self.vid_pop[vid] for vid in vids], dtype=np.float64)
            probs_dist = np.array([np.exp(-vid_dist[vid]) for vid in vids], dtype=np.float64)
            probs_pop /= probs_pop.sum()
            probs_dist /= probs_dist.sum()
            self.rid_sampling_info[rid] = (vids, probs_pop, probs_dist)
        if is_train:
            if random.random() < 0.5:
                id_cnt = np.random.multinomial(self.nb_cnt, probs_pop)
            else:
                id_cnt = np.random.multinomial(self.nb_cnt, probs_dist)
...
def test(dl,root_path, dataset, iter_start=0, mod=0):
    torch.manual_seed(0)
    random.seed(0)
    #dl = pickle.load(open(root_path + 'dl_' + dataset + '.pk', 'rb'))
    for _, records_u in dl.uid_records.items():
    ...
    for iter in range(iter_start, 0, -5):
        model = AttentionModelNew(dl.nu, dl.nv, dl.nt, sampling_list=dl.sampling_list, vid_coor_nor=dl.vid_coor_nor,
                               vid_pop=dl.vid_pop, mod=mod)
        if iter_start != 0:
            model.load_state_dict(torch.load(root_path + 'model_attention_nobias_new_' + str(mod) + '_' + str(iter) + '.md'))
        hits = np.zeros(3)
        cnt = 0
